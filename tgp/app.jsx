...
function App() {
  ...
  const goBackNav = (fallback = "home") => {
    setNavHistory((h) => {
      if (h.length === 0) {setScreen(fallback);return h;}
  ...
  const handleSaveNames = (dad, partner) => {
    if (dad) setDadName(dad);
    if (partner) setPartnerName(partner);
  };

  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--cream)", position: "relative" }}>
      <StatusBar />
      <div className="screen-area">
        {screen === "home" &&
        <Home
          bAnswers={bAnswers} bagIncluded={bagIncluded} bagCustom={bagCustom}
          bagPacked={bagPacked} bagInterviewDone={bagInterviewDone}
          dadName={dadName} partnerName={partnerName}
          onStart={(t) => navigate(t === "birth" ? "birth" : t === "birth-summary" ? "birth-summary" : t === "bag-summary" ? "bag-summary" : "bag")}
          onOpenOverflow={() => setShowOverflow(true)}
          onReset={() => setShowResetConfirm(true)} />

        }
        {screen === "birth" &&
        <BirthInterview answers={bAnswers} onAnswer={handleAnswer}
        onBack={() => goBackNav("home")} onFinish={() => navigateReplace("birth-summary", ["home"])}
        editQuestionId={editQuestionId} onEditDone={() => {setEditQuestionId(null);goBackNav("birth-summary");}} />

        }
        {screen === "birth-summary" &&
        <BirthSummary answers={bAnswers} onBack={() => goBackNav("home")}
        onHome={() => resetNav("home")}
        onEditQuestion={(id) => {setEditQuestionId(id);navigate("birth");}} />

        }
        {screen === "bag" &&
        <BagInterview included={bagIncluded} onToggleIncluded={toggleIncluded}
        customItems={bagCustom} onAddCustom={addCustom} onRemoveCustom={removeCustom}
        onBack={() => goBackNav("home")}
        onFinish={() => {setBagInterviewDone(true);navigateReplace("bag-summary", ["home"]);}}
        initialCatIdx={bagEditCatIdx} />

        }
        {screen === "bag-summary" &&
        <BagSummary included={bagIncluded} customItems={bagCustom} packed={bagPacked}
        onTogglePacked={togglePacked}
        onEditCat={goToBagCat}
        onBack={() => goBackNav("home")} onHome={() => resetNav("home")} />

        }
      </div>
      <div className="home-indicator"><div className="home-bar" /></div>

      {/* ── First-launch / replay onboarding ── */}
      {(showOnboarding || replayOnboarding) &&
      <FirstLaunchModal onComplete={handleOnboardingComplete} />
      }
      ...
      <OverflowMenu
        ...
        onReviewOnboarding={() => setReplayOnboarding(true)}
        onResetBirth={doResetBirth}
        onResetBag={doResetBag}
        onResetAll={doReset}
      ...
      {/* ── Reset confirm sheet ── */}
      {showResetConfirm &&
      <div className="confirm-scrim" style={{ zIndex: 400 }}>
          <div className="confirm-sheet">
            <div className="confirm-title">Start over?</div>
            <div className="confirm-body">This will clear all your birth plan answers and bag selections. This cannot be undone.</div>
            <div className="confirm-btns">
              <button className="confirm-btn-dest" onClick={doReset} aria-label="Confirm reset, clear all data">Yes, start fresh</button>
              <button className="confirm-btn-stay" onClick={() => setShowResetConfirm(false)} aria-label="Cancel reset">Keep my progress</button>
            </div>
          </div>
        </div>
      }
